...
Public Class ReRegisterHDMWindow

    Friend iStatus As String
    Friend iRegion As String
    Friend iTesuch As String
    Friend iOldHvhh As String
    Dim OldHVHH As String
    ...
    Dim GP As New GPS With {.Latitude = String.Empty, .Longitude = String.Empty}
    ...
    Private Sub LoadDT()
        ...
        OldHVHH = txtHvhh.Text.Trim
    End Sub
    Private Sub LoadGPS(ByVal ecr As String)
        On Error Resume Next

        Dim dt As DataTable = iDB.RetExtGPSCo(ecr)
        If dt.Rows.Count = 1 Then
            GP.Latitude = dt.Rows(0)("Latitude")
            GP.Longitude = dt.Rows(0)("Longitude")
        Else
            GP.Latitude = String.Empty
            GP.Longitude = String.Empty
        End If
    End Sub
    ...
    Private Sub ClearData()
        ...
        txtEcr.Focus()
    End Sub
    Private Sub ReRegisterHDMWindow_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        Try
            ...
            If txtHvhh.Text.Trim.Length < 8 OrElse txtHvhh.Text.Trim.Length > 9 Then Throw New Exception("ՀՎՀՀ-ն պետք է ունենա 8/9 սիմվոլ երկարություն")
            If txtMgh.Text.Trim.Length <> 8 Then Throw New Exception("ՄԳՀ-ն պետք է ունենա 8 սիմվոլ երկարություն")

            If txtHvhh.Text.Trim = iOldHvhh Then Throw New Exception("ՀՎՀՀ-ն չի կարող նույնը մնալ")
            ...
            'Get GPS Coordinats
            Call LoadGPS(iEcr.Ecr)

            'Change GPS Coordinates
            Dim fG As New GPSForEcrRep With {.Ecr = iEcr.Ecr, .GPSX = GP}
            fG.ShowDialog()
            Dim RES As Boolean = fG.IsOK
            fG.Dispose()

            If RES = False Then Throw New Exception("GPS կոորդինատները չեն հաստատվել")

            Dim checked As Boolean?
            If OldHVHH = "01256686" Then checked = 1
            If txtHvhh.Text = "01256686" Then checked = 0

            'ReRegister Ecr
            iDB.ReRegisterEcr(iEcr.Ecr, iEcr.hvhh, iEcr.mgh, iEcr.ContDate, iEcr.gorcAddress, iEcr.gprs, iEcr.status, iEcr.regionID, iEcr.objType,
                  iEcr.Tel, iEcr.tesuch, iEcr.IsNds, iEcr.posTerm, iEcr.isInvoice, checked)

            'Dim dt As DataTable = iDB.ShuldPrintReRegInvoice(iOldHvhh)

            'If dt.Rows(0)(0) = 0 Then
            '    MsgBox("Գործընկերը այլևս ՀԴՄ չունի, հարկավոր է տպել հաշիվ ապրանքագիր", MsgBoxStyle.Exclamation, My.Application.Info.Title)
            'End If
    ...
    Private Sub cbRegion_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbRegion.SelectedIndexChanged
        ...
        Next
    End Sub
    Private Sub ReRegisterHDMWindow_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Call LoadDT()
    End Sub
